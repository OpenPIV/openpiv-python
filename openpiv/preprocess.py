...
def offset_image(img, offset_x, offset_y):
    """
...
def stretch_image(img,
    ...
    resize: int
        if not None, normalizes the image and resizes the max intensity
    ...
    img = rescale(img, (y_axis, x_axis))
    if resize != None: # Why did is it done like this?
        img *= resize
    return img