...
def imread( filename, flatten=0 ):
    ...
    """
    if filename.endswith(".im7"):
        return __readim7(filename)
    else:
        return io.imread(filename, as_grey = True)

def __readim7(filename):
    """
    read im7 files from LaVision's Davis software
    returns:
       -two frames of images
       -PIV parameters: scaling, dt etc...

    https://bitbucket.org/fleming79/readim/overview
    http://jekel.me/2015/Open-and-View-IM7-Files-with-Python/
    """
    buff, i_atts   =  ReadIM.extra.get_Buffer_andAttributeList(filename)
    i_array, i_buff = ReadIM.extra.buffer_as_array(buff)

    #clean the memory
    del(buff)
    ReadIM.DestroyBuffer(i_buff)
    ReadIM.DestroyAttributeListSafe(i_atts)

    return i_array[0],i_array[1]
