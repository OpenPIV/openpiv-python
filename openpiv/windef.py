...
def process_image_pair(args, settings, save_path):
...
class _PIVWorker:
    """Picklable worker callable for multiprocessing PIV evaluation across image pairs."""

    def __init__(self, settings: "PIVSettings", save_path: pathlib.Path):
        self.settings = settings
        self.save_path = save_path

    def __call__(self, args):
        return process_image_pair(args, self.settings, self.save_path)


def piv(settings: "PIVSettings"):
    """The main entry point in which batch PIV evaluation is performed."""
    # if the settings.save_path is a string convert it to Path
    ...
    n_cpus = getattr(settings, "n_cpus", 1)
    worker = _PIVWorker(settings, save_path)
    task.run(func=worker, n_cpus=n_cpus)
...
def multigrid_windef(
    frame_a: np.ndarray,
    frame_b: np.ndarray,
    settings: Optional["PIVSettings"] = None,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Multigrid (multi-pass) window deformation PIV evaluation on image arrays.

    Iteratively performs coarse-to-fine interrogation with window deformation,
    vector validation, and outlier replacement across grid resolutions.
    ...
    frame_b : np.ndarray
        Second image frame.
    settings : Optional[PIVSettings], optional
        PIV configuration settings specifying window sizes, overlap,
        correlation backend ('scipy' or 'rust'), and validation parameters.
    ...
    x, y : np.ndarray
        Grid coordinates.
    u, v : np.ndarray
        Velocity displacement components.
    flags : np.ndarray
        Boolean validation mask (0 = valid, 1 = invalid / interpolated).
    """
    return simple_multipass(frame_a, frame_b, settings)
