...
from windef import first_pass as first
from windef import multipass_img_deform as multi
...
def test_first_pass_circ():
    ...
    x, y, u, v, sig2noise_ratio= first(frame_a, frame_b, window_size=64, overlap=32,iterations=1,correlation_method='circular',
...
def test_multi_pass_circ():
    ...
    with warnings.catch_warnings():
        ...
        frame_a = img_as_ubyte(frame_a)
    frame_b = np.roll(np.roll(frame_a,3,axis=1),2,axis=0)
    x, y, u, v, sig2noise_ratio= first(frame_a, frame_b, window_size[0], overlap[0],iterations,correlation_method='circular', subpixel_method='gaussian',do_sig2noise=True, sig2noise_method='peak2peak', sig2noise_mask=2)
    u_old=u
    v_old=v
    i=1
    for i in range(2,iterations+1):
        x, y, u, v, sig2noise_ratio, mask=multi(frame_a, frame_b, window_size[i-1], overlap[i-1],iterations,i, x, y, u, v, correlation_method='circular',
...
def test_first_pass_lin():
    ...
    with warnings.catch_warnings():
        ...
        frame_a = img_as_ubyte(frame_a)
    frame_b = np.roll(np.roll(frame_a,3,axis=1),2,axis=0)
    x, y, u, v, sig2noise_ratio= first(frame_a, frame_b, window_size=64, overlap=32,iterations=1,correlation_method='linear',
                                       subpixel_method='gaussian',do_sig2noise=True, sig2noise_method='peak2peak', sig2noise_mask=2)
    # print u,v
    #print('test')
    assert(np.max(np.abs(u-3)) < 0.1)
...
def test_multi_pass_lin():
    ...
    with warnings.catch_warnings():
        ...
        frame_a = img_as_ubyte(frame_a)
    frame_b = np.roll(np.roll(frame_a,3,axis=1),2,axis=0)
    x, y, u, v, sig2noise_ratio= first(frame_a, frame_b, window_size[0], overlap[0],iterations,correlation_method='linear', subpixel_method='gaussian',do_sig2noise=True, sig2noise_method='peak2peak', sig2noise_mask=2)
    u_old=u
    v_old=v
    i=1
    for i in range(2,iterations+1):
        x, y, u, v, sig2noise_ratio, mask=multi(frame_a, frame_b, window_size[i-1], overlap[i-1],iterations,i, x, y, u, v, correlation_method='linear',
                             subpixel_method='gaussian', do_sig2noise=False, sig2noise_method='peak2peak', sig2noise_mask=2,
                             MinMaxU=(-100, 50), MinMaxV=(-50, 50), std_threshold=1000000, median_threshold=200000,median_size=1, filter_method='localmean',
                             max_filter_iteration=10, filter_kernel_size=2, interpolation_order=3)
    assert(np.max(np.abs(u-3)) < 0.1 and np.any(u!=u_old))
